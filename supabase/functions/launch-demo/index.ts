...
const SEED_DAYS: SeedDay[] = [
    ...
    activities: [
      {
        ...
        description: "Approach the ancient Buddhist temple through Kaminarimon gate before the tourists arrive. Watch the morning rituals and enjoy the incense-filled courtyard in near-solitude.",
      ...
      },
      {
        ...
        description: "Browse 50+ traditional craft and souvenir stalls along the 250-meter approach to Senso-ji. Try ningyo-yaki (doll-shaped cakes) and pick up authentic Japanese gifts.",
      ...
      },
      {
        ...
        description: "Walk through 14 buildings of Japanese art, archaeology, and cultural artifacts spanning 10,000 years. Don't miss the Honkan main hall.",
  ...
  },
  {
    ...
    summary: "From serene forest shrines to the world's busiest crossing — today captures Tokyo's fascinating collision of ancient ritual and modern spectacle.",
    activities: [
      {
        title: "Meiji Jingu Shrine Morning Walk",
        description: "Follow a 700-meter forested path through 70,000 trees to reach one of Japan's most important Shinto shrines. Observe the peaceful morning purification rituals.",
      ...
      },
      {
        ...
        description: "Walk the famous 350-meter pedestrian street lined with over-the-top fashion boutiques, vintage stores, and legendary crepe shops serving outrageous sweet creations.",
        ...
        estimated_cost: 20,
        category: "Food",
        ai_reason: "Takeshita Street is a hidden gem into Tokyo's youth subculture — the fashion boutiques and crepe shops reveal a side of contemporary Japanese identity you won't find elsewhere.",
      },
      {
        ...
        estimated_cost: 18,
        category: "Food",
        ai_reason: "Shibuya Crossing is most dramatic at midday when up to 3,000 pedestrians cross at once. Paired with a standing ramen bar next door, this is the quintessential modern Tokyo experience.",
      },
    ],
  },
  {
    ...
    activities: [
      ...
      },
      {
        ...
        description: "Explore large rooms of borderless digital art that flows, moves, and reacts to your presence. One of Tokyo's most unique and unmissable cultural experiences.",
        ...
        estimated_cost: 35,
        category: "Culture",
        ai_reason: "TeamLab Borderless is a world-famous digital art museum combining cutting-edge technology with culture. Only 25 min from central Tokyo by Yurikamome train.",
      },
    ],
  },
  {
    ...
    activities: [
      {
        ...
        description: "Start your last morning at this lively open-air market under the elevated train tracks in Ueno. Grab breakfast from the food stalls: grilled squid, yakitori, fresh fruit.",
        ...
        estimated_cost: 20,
        category: "Food",
        ai_reason: "Ameyoko opens early for a budget-friendly final breakfast. The market's energy and variety make for a memorable last Tokyo morning before airport check-in.",
      },
      {
...
async function seedDemoTrip(
  ...
  if (tripErr || !tripData) {
    console.error("[launch-demo] trip insert:", tripErr?.message);
    throw new Error("Failed to create demo trip.");
  }
  ...
  for (let dayIdx = 0; dayIdx < SEED_DAYS.length; dayIdx++) {
    ...
    if (dayErr || !dayData) {
      console.error("[launch-demo] day insert:", dayErr?.message);
      continue;
    }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Create a unique demo user per launch — gives each visitor full data isolation
    ...
    if (createErr || !newUser?.user) {
      console.error("[launch-demo] createUser:", createErr?.message);
      return jsonRes({ error: "Could not create demo session. Please try again." }, 500);
    }

    const userId = newUser.user.id;

    // Sign in to get session tokens — call auth endpoint directly
    ...
    if (!tokenRes.ok) {
      console.error("[launch-demo] token exchange failed:", tokenRes.status);
      return jsonRes({ error: "Could not start demo session. Please try again." }, 500);
    }
    ...
    // Seed the demo trip with service role (bypasses RLS — userId verified above)
    const tripId = await seedDemoTrip(adminClient, userId);

    console.log(`[launch-demo] Demo user ${userId} created with trip ${tripId}`);
    ...
    console.error("[launch-demo] Unhandled error:", err);
